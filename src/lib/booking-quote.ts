...
export type BookingQuoteSuccess = {
  ok: true;
  type: 'single' | 'multi';
  nights: number;
  total_amount: number;
...
export type BookingQuoteResult = BookingQuoteSuccess | BookingQuoteError;
...
async function quoteSingle(draft: Extract<BookingDraft, { type: 'single' }>): Promise<BookingQuoteResult> {
  const nights =
    draft.display?.number_of_nights ?? calculateNights(draft.payload.check_in, draft.payload.check_out);
  if (nights <= 0) {
    return { ok: false, error: 'Ngày nhận/trả phòng không hợp lệ', status: 400 };
  }

  const branchCode =
    (typeof draft.payload.branch_code === 'string' && draft.payload.branch_code.trim()) ||
    DEFAULT_BRANCH_CODE;
  const resolvedBranchId = await resolveBranchIdForFilter(supabase, null, branchCode);
  ...
  if (draft.payload.room_id) {
    ...
    if (data) room = data as typeof room;
    ...
    if (data && data.length > 0) room = data[0] as typeof room;
  }

  let branch: { code: string; name: string } | null = null;
  if (resolvedBranchId) {
    const { data: branchRow } = await supabase
      ...
      .maybeSingle();
    if (branchRow) {
      branch = { code: branchRow.code, name: branchRow.name };
    }
  }
  ...
  try {
    ...
    if (settings && 'pricing_weekday_rates' in settings && (settings as { pricing_weekday_rates?: unknown }).pricing_weekday_rates) {
      weekdayRates = normalizeWeekdayRates((settings as { pricing_weekday_rates: unknown }).pricing_weekday_rates);
    }
...
async function quoteMulti(draft: Extract<BookingDraft, { type: 'multi' }>): Promise<BookingQuoteResult> {
  const nights =
    draft.payload.number_of_nights ?? calculateNights(draft.payload.check_in, draft.payload.check_out);
  if (nights <= 0) {
    return { ok: false, error: 'Ngày nhận/trả phòng không hợp lệ', status: 400 };
  }
  ...
  const total = (draft.payload.room_items ?? []).reduce(
    (sum, i: { amount?: number }) => sum + (Number(i.amount) || 0),
    0
  );
  return {
    ok: true,
...
export async function computeBookingQuote(draft: BookingDraft): Promise<BookingQuoteResult> {
  if (draft.type === 'single') return quoteSingle(draft);
  return quoteMulti(draft);
}
